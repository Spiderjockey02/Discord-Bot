  ...
  if (message.deletable) message.delete();
	...
	if (settings.ModLogEvents.includes('report')) {
	  ...
	  var embed = new Discord.MessageEmbed()
		...
		const channel = message.guild.channels.cache.find(channel => channel.name === settings.ModLogChannel);
	  if (channel) {
			channel.send(embed).catch(e => bot.logger.error(`Missing access to \'${settings.ModLogChannel}\' in server: [${message.guild.id}]`))
