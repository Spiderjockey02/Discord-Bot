...
module.exports.run = async (bot, message, args, settings, ops) => {
  ...
  var r = await message.channel.send("Gathering playlist information")
  if (urlValid) {
    ...
    if (args[0].includes('/sets/')) {
      try {
        var playlist = await scdl.getSetInfo(args[0], bot.config.soundcloudAPI_Key)
        //console.log(playlist)
        var videos = playlist.tracks.map(track => ({
          title: track.title,
    ...
    var r = await getData(args[0])
    var videos = r.tracks.items
