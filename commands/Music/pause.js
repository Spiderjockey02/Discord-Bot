module.exports.run = async (bot, message, args, settings, ops) => {
	...
	if (fetched.dispatcher.paused) return message.channel.send({embed:{color:15158332, description:`${bot.config.emojis.cross} I am already paused, \`!resume\` to carry on listening.`}}).then(m => m.delete({ timeout: 10000 }))
