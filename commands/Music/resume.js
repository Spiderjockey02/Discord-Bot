module.exports.run = async (bot, message, args, settings, ops) => {
	...
	if (!fetched.dispatcher.paused) return  message.channel.send({embed:{color:15158332, description:`${bot.config.emojis.cross} I am already playing, \`!pause\` to pause song.`}}).then(m => m.delete({ timeout: 10000 }))
