module.exports.run = async (bot, message, args, emoji, settings, ops) => {
	...
	if (message.member.voiceChannel !== message.guild.me.voiceChannel) return message.channel.send({ embed:{ color:15158332, description:`${emoji} Sorry, you currently aren't in the same channel as the bot.` } }).then(m => m.delete({ timeout: 5000 }));
	if (!args[0]) return message.channel.send({ embed:{ color:15158332, description:`${emoji} Please use the format \`${bot.commands.get('loop').help.usage.replace('${PREFIX}')}\`.` } }).then(m => m.delete({ timeout: 5000 }));
	if (args[0].toLowerCase() == 'song') {
		...
		return message.channel.send({ embed:{ color:15158332, description:`${emoji} Please use the format \`${bot.commands.get('loop').help.usage.replace('${PREFIX}', settings.prefix)}\`.` } }).then(m => m.delete({ timeout: 5000 }));
	}
};
