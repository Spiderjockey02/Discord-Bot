  ...
  */
	constructor() {
		super({
			...
			slash: true,
			options: client.subCommands.filter(c => c.help.category == 'Giveaway' && c.help.name !== 'giveaway').map(c => ({
	...
	*/
	async callback(client, interaction, guild, args) {
		const command = client.subCommands.get(`g-${interaction.options.getSubcommand()}`);
		if (command) {
