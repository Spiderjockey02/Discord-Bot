module.exports.run = async (bot, message, args, emojis, settings, ops) => {
	...
	// Find what to remove
	if (args.length == 0) return message.channel.send({ embed:{ color:15158332, description:`${emojis[0]} Please use the format \`${bot.commands.get('remove').help.usage.replace('${PREFIX}', settings.prefix)}\`.` } }).then(m => m.delete({ timeout: 5000 }));
	// Position
	if (!isNaN(args[0])) {
		...
		message.channel.send({ embed:{ color:3066993, description:`${emojis[1]} Successfully removed \`${fetched.queue[args[0]].title}\` from queue.` } });
		fetched.queue.splice(args[0], 1);
	} else {
		return message.channel.send({ embed:{ color:15158332, description:`${emojis[0]} Please use the format \`${bot.commands.get('remove').help.usage.replace('${PREFIX}', settings.prefix)}\`.` } }).then(m => m.delete({ timeout: 5000 }));
	}
};
