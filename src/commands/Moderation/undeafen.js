...
module.exports = class Undeafen extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.member.permissions.has('DEAFEN_MEMBERS')) return message.channel.error(settings.Language, 'USER_PERMISSION', 'DEAFEN_MEMBERS').then(m => setTimeout(() => { m.delete(); }, 10000));
		...
		try {
			...
			message.channel.success(settings.Language, 'MODERATION/SUCCESSFULL_UNDEAFEN', member[0].user).then(m => setTimeout(() => { m.delete(); }, 3000)
);
		} catch (err) {
