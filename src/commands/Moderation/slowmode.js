...
module.exports = class SlowMode extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.member.permissions.has('MANAGE_CHANNELS')) return message.channel.error(settings.Language, 'USER_PERMISSION', 'MANAGE_CHANNELS').then(m => setTimeout(() => { m.delete(); }, 10000));
