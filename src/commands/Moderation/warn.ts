...
export default class Warn extends Command {
	...
	*/
	async run(client, message, settings) {
		...
		try {
			// client, message, member, wReason, settings
			const res = await require('../../helpers/warningSystem').run(client, message, members[0], reason);
	...
	*/
	async callback(client, interaction, guild, args) {
		...
		try {
			const res = await require('../../helpers/warningSystem').run(client, interaction, member, reason);
