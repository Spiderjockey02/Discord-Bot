...
module.exports = class Premium extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.args[2]) return message.channel.error(settings.Language, 'INCORRECT_FORMAT', settings.prefix.concat(this.help.usage)).then(m => setTimeout(() => { m.delete(); }, 5000));
