...
class ChangeMyMind extends Command {
	...
	*/
	async run(bot, message, settings) {
		...
		if (!text) return message.channel.error('misc:INCORRECT_FORMAT', { EXAMPLE: settings.prefix.concat(message.translate('image/changemymind:USAGE')) }).then(m => m.timedDelete({ timeout: 5000 }));

		// make sure the text isn't longer than 80 characters
		if (text.length >= 81) return message.channel.error('image/changemymind:TOO_LONG').then(m => m.timedDelete({ timeout: 5000 }));
