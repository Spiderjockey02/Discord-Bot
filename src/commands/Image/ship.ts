...
export default class Ship extends Command {
	...
	*/
	async run(client, message, settings) {
		...
		try {
			const resp = await client.fetch('image/distracted', { image1: files[0], image2: files[1] });
	...
	*/
	async callback(client, interaction, guild, args) {
		...
		try {
			const resp = await client.fetch('image/distracted', { image1: member.user.displayAvatarURL({ format: 'png', size: 1024 }), image2:  member2.user.displayAvatarURL({ format: 'png', size: 512 }) });
