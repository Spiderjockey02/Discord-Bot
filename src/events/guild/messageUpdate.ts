...
export default class MessageUpdate extends Event {
	...
	*/
	async run(client: EgglordClient, oldMessage: Message, newMessage: Message) {
		// For debugging
		client.logger.debug(`Message updated${!newMessage.guild ? '' : ` in guild: ${newMessage.guild.id}`}`);
		...
		const embed = new EgglordEmbed(client, newMessage.guild)
			.setDescription(`**Message of ${newMessage.author} edited in ${newMessage.channel}** [Jump to Message](${newMessage.url})`)
