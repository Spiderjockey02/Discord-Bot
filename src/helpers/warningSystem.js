...
module.exports.run = async (bot, userInput, member, reason) => {
	...
	try {
		...
		if (!warnings[0]) {
			// debugging mode
			if (bot.config.debug) bot.logger.debug(`${member.user.tag} was warned for the first time in guild: ${userInput.guild.id}`);
			...
			if (warnings.length == 1) {
				...
				const embed1 = new Embed(bot, guild)
					...
					.setAuthor({ name: guild.translate('moderation/warn:SUCCESS', { USER: member.user.tag }), iconURL: member.user.displayAvatarURL() })
					.setDescription(guild.translate('moderation/warn:REASON', { REASON: reason }));
				if (bot.config.debug) bot.logger.debug(`${member.user.tag} was warned for the second time in guild: ${guild.id}`);
				...
				if (bot.config.debug) bot.logger.debug(`${member.user.tag} was warned for the third time in guild: ${guild.id}`);
