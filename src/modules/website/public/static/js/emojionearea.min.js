...
function(a, b) {
	...
	let h = 'data:image/gif;base64,R0lGODlhAQABAJH/AP///wAAAMDAwAAAACH5BAEAAAIALAAAAAABAAEAAAICVAEAOw==',
		i = [].slice,
		j = 'emojionearea',
		k = 0,
		l = '&#8203;';
	...
	function B(a, b) {
		return a = a.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#x27;').replace(/`/g, '&#x60;').replace(/(?:\r\n|\r|\n)/g, '\n').replace(/(\n+)/g, '<div>$1</div>').replace(/\n/g, '<br/>').replace(/<br\/><\/div>/g, '</div>'), b.shortnames && (a = e.shortnameToUnicode(a)), A(a, b.emojiTemplate).replace(/\t/g, '&nbsp;&nbsp;&nbsp;&nbsp;').replace(/ {2}/g, '&nbsp;&nbsp;');
	}
	...
	function I(b, c) {
		...
		if (!b.recent || b.recent !== d || c) {
			if (d.length) {
				let e = b.scrollArea.is('.skinnable'),
					f, g;
	...
	function L(b, c, d) {
		...
		let g = c.is('TEXTAREA') || c.is('INPUT') ? 'val' : 'text',
		...
		if (M.on('scroll', function() {
			...
			return a(d.target).hasClass('search') ? (b.stayFocused = !0, b.searchSel === null && (b.searchSel = x(c[0]))) : (O.is('.focused') || c.trigger('focus'), d.preventDefault()), !1;
		}).on('@change', function() {
			...
			g && g.length ? (b.recentFilter.hasClass('active') && b.recentFilter.removeClass('active').next().addClass('active'), b.recentCategory.hide(), b.recentFilter.hide(), L.each(function() {
				const b = function(a, b) {
						...
						if (c.length === 0) {a.data('tone') === b && a.hide(), e.filter('[data-filter="' + a.attr('name') + '"]').hide();} else {
			...
			const S = function() {
					...
					const f = a.map(e.emojioneList, function(a, b) {
